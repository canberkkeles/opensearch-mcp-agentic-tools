...
This module provides functions to initialize OpenSearch clients using
environment-variable-based configuration for a single target cluster.
"""
...
try:
    _VERSION = importlib.metadata.version('opensearch-agent-tools-mcp')
except importlib.metadata.PackageNotFoundError:
    _VERSION = 'unknown'
USER_AGENT = f'opensearch-agent-tools-mcp/{_VERSION}'
...
def initialize_client(args: baseToolArgs) -> AsyncOpenSearch:
    """Initialize and return an OpenSearch client for the configured cluster.
    ...
    try:
        del args
        logger.info('Initializing OpenSearch client in single-cluster mode')
...
def _initialize_client_single_mode() -> AsyncOpenSearch:
    ...
    Single-cluster mode uses environment variables for connection, with optional header-based auth:
    ...
    try:
        ...
        logger.info(f'Initializing OpenSearch client for URL: {opensearch_url}')
        ...
        logger.error(f'Unexpected error in OpenSearch client initialization: {e}')
        raise ConfigurationError(f'Failed to initialize OpenSearch client: {e}')
...
def get_aws_region_single_mode() -> Optional[str]:
    """Get AWS region using environment variables and boto3 defaults.
    ...
    try:
        ...
        logger.error(f'Unexpected error getting AWS region: {e}')
        return None
